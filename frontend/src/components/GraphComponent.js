...
const GraphComponent = ({ userid, getWallets, getSpendings }) => {
  ...
  const handleChange = async (e) => {
    ...
    const data = {
      xAxis: [{ scaleType: "band", data: Object.keys(categoryTotals) }],
      series: Object.keys(categoryTotals).map((category) => ({
  ...
  return (
    <Container className="border border-3 rounded mt-3 p-3">
      <Row>
        <h1 className="text-muted">Graphs{"    "}</h1>
      </Row>
      <Row>
        <Col md={4}>
      ...
      <Row>
        {chartData && chartData.xAxis.length > 0 && chartData.series.length > 0 ? (
          <BarChart
